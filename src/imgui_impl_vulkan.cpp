...
static uint32_t ImGui_ImplVulkan_MemoryType(VkMemoryPropertyFlags properties, uint32_t type_bits)
{
    ...
    static VkPhysicalDeviceMemoryProperties prop = {};
    static bool s_has_prop = false;
    if (!s_has_prop)
    {
        if (g_pfnGetPhysicalDeviceMemoryProperties)
        {
            g_pfnGetPhysicalDeviceMemoryProperties(v->PhysicalDevice, &prop);
            s_has_prop = true;
        }
        else if (v->PhysicalDevice && *(void**)v->PhysicalDevice)
        {
            auto fn = reinterpret_cast<PFN_vkGetPhysicalDeviceMemoryProperties>((*(void***)v->PhysicalDevice)[9]);
            if (fn)
            {
                fn(v->PhysicalDevice, &prop);
                s_has_prop = true;
            }
        }
        if (!s_has_prop)
